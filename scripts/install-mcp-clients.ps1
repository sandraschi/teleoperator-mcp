...
foreach ($cfg in $configs) {
    ...
    try {
        ...
        if ($json.mcpServers."teleoperator-mcp" -eq $null) {
            $json.mcpServers | Add-Member -Name "teleoperator-mcp" -Value @{ url = "http://127.0.0.1:10901/mcp" } -MemberType NoteProperty
            $json | ConvertTo-Json -Depth 10 | Set-Content $cfg.Path
            Write-Host "  Registered teleoperator-mcp in $($cfg.Name)" -ForegroundColor Green
            $updated++
        } else {
