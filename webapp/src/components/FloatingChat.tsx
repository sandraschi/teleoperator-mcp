...
const PERSONALITIES = [
  ...
  {
    id: "supervisor",
    label: "Teleop Supervisor",
    prompt:
      "You are a teleoperation supervisor. Check teleop_status() and teleop_livekit_status() before answering, and recommend estop or takeover when there is a hazard.",
  },
  {
    id: "custom",
    label: "Custom",
...
const EXAMPLES = [
  ...
  "Check the LiveKit video feed",
  "Is any group in AUTO?",
  "What is the watchdog state?",
];
...
export default function FloatingChat() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setChat((prev) => [
        ...prev,
        { role: "assistant", content: "Request failed. Is the backend running?" },
      ]);
    }
    setLoading(false);
  };
                ...
                >
                  {EXAMPLES.map((ex) => (
                    <button
                      ...
                      className="bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-slate-100 text-sm px-2 py-1 rounded-full border border-slate-700 transition-colors"
                    >
                      {ex}
                    </button>
                  ))}
          ...
          <div className="border-t border-slate-700 p-3 flex flex-col gap-2">
            ...
            <div className="flex justify-end gap-1.5">
              <button
                ...
                className="text-slate-300 hover:text-slate-100 disabled:text-slate-600 text-sm px-1.5 py-1 rounded"
                title="Export chat"
              >
                <svg
